...
% yeomyung
% [powerControl]
simParams = addNewParam(simParams,'powerControl',true,'powerControl (T/F)','bool',fileCfg,varargin{1}{1});
% [rateControl]
simParams = addNewParam(simParams,'rateControl',true,'rateControl (T/F)','bool',fileCfg,varargin{1}{1});

% [printLOG]
simParams = addNewParam(simParams,'printLOG',true,'printLOG (T/F)','bool',fileCfg,varargin{1}{1});

% [ITTpercent]
simParams = addNewParam(simParams,'ITTpercent',100,'ITTpercent (100/80/60/40/20)','integer',fileCfg,varargin{1}{1});

% [simulationStep]
simParams = addNewParam(simParams,'simulationStep',0.001,'Simulation Step (s)','double',fileCfg,varargin{1}{1});
if simParams.simulationStep<=0
    error('Error: "simParams.simulationStep" cannot be <= 0');
end
...
%yeomyung
% [sensingPwr]
simParams = addNewParam(simParams,'sensingPwr',-140,'sciFeedback','double',fileCfg,varargin{1}{1});


%yeomyung
% [CBRThresh]
simParams = addNewParam(simParams,'CBRThresh',-107,'CBR Threshold','integer',fileCfg,varargin{1}{1});

% [CBRRange]
simParams = addNewParam(simParams,'CBRRange',100,'CBR Range','integer',fileCfg,varargin{1}{1});
